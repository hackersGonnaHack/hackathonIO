...
class ViewController: UIViewController,WKNavigationDelegate {
    ...
    override func viewDidLoad() {
        ...
        keyChain.set("\(230)", forKey: "tirepressurefrontleft")
        keyChain.set("\(230)", forKey: "tirepressurefrontright")
        keyChain.set("\(230)", forKey: "tirepressurerearleft")
        keyChain.set("\(230)", forKey: "tirepressurerearright")
        
        keyChain.set("\(80)", forKey: "fuellevelpercent")

        keyChain.set("CLOSED", forKey: "doorstatusrearright")
        keyChain.set("CLOSED", forKey: "doorstatusrearleft")
        keyChain.set("CLOSED", forKey: "doorstatusfrontright")
        keyChain.set("CLOSED", forKey: "doorstatusfrontleft")
        

        self.view.addSubview(webView)
        self.view.sendSubviewToBack(webView)

        //self.getInfo(type: self.getType, url2: "https://api.mercedes-benz.com/experimental/connectedvehicle/v1/vehicles/")
    ...
    func lockCar( url2:String)
    {
        let carID:String = "DB4D2536FE75E7CF5F"
        
        
        
        let url:String = url2 + carID + "/doors"
        ...
        let parameters : Parameters = ["command": "LOCK"]
    ...
    func unlockCar( url2:String)
    {
        ...
        let parameters : Parameters = ["command": "UNLOCK"]
        ...
        for (key, value) in self.dictResp {
            ...
            case "Additional Info":
                if String(describing: key) == "colorname"
                {
                    ...
                    keyChain.set("\(value)", forKey: "colorname")
            ...
            case "Tire":
                ...
                let cs = String(s.dropFirst())
                let ccs = String(cs.dropLast())
                let ccs1 = ccs.components(separatedBy: ";")
                
                let tirePressure  = ccs1[3]
                
                keyChain.set("\(tirePressure)", forKey: "\(key)")
                
                
            case "Fuel":
                if String(describing: key) == "value"
                {
                    print("Fuel = \(value)")
                    keyChain.set("\(value)", forKey: "fuel")
                }
            ...
            case "Door":
                ...
                let doorValue  = ccs1[2]
                keyChain.set("\(doorValue)", forKey: "\(key)")
    ...
    func refreshToken()
    {
        let rToken = keyChain.get("refresh_token")
        let clientIDSecret = "1880dea4-642b-4f11-9803-dafb428a8ae1:771a61f9-1af3-4aa2-a300-3b14af2d4a06"
        let encoded = clientIDSecret.toBase64()
        
        let headers: HTTPHeaders = ["authorization": "Basic \(encoded)","content-type":"application/x-www-form-urlencoded"]
        let parameters : Parameters = ["grant_type": "refresh_token","code":"\(String(describing: rToken)))","redirect_uri":"https://s3.us-east-2.amazonaws.com/mercedes-marsters-bucket/index.html"]
        print(parameters)
        let exchangeUrl = "https://api.secure.mercedes-benz.com/oidc10/auth/oauth/v2/token"
